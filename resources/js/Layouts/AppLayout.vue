    ...
    <div class="min-h-screen bg-black font-sans text-white selection:bg-red-600 selection:text-white">
        ...
        <nav :class="[
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center">
                    ...
                    <Link href="/" class="group z-50 relative flex flex-col">
                        <span class="text-3xl font-black tracking-tighter text-white transition-colors duration-300">
                            f<span class="text-red-600">3</span>3
                        </span>
                        <span class="text-[0.55rem] uppercase tracking-[0.4em] text-gray-500 transition-colors duration-300 -mt-1">
                            Photography
                        </span>
                    </Link>
                    ...
                    <div class="hidden md:flex items-center space-x-10">
                        ...
                        <div class="flex items-center space-x-6">
                            <Link v-if="user" :href="route('cart.index')" class="relative group flex items-center justify-center w-8 h-8 rounded-full hover:bg-zinc-800 transition-colors duration-300">
                                <ShoppingCartIcon class="w-4 h-4 text-gray-400 group-hover:text-red-600 transition-colors duration-300" />
                                <span v-if="cartCount > 0" class="absolute -top-1 -right-1 min-w-[1.1rem] h-[1.1rem] flex items-center justify-center px-1 rounded-full text-[9px] font-bold bg-red-600 text-white">
                                    {{ cartCount > 99 ? '99+' : cartCount }}
                                </span>
                            </Link>

                            <template v-if="!user">
                                <Link :href="route('photographer.register')" class="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 hover:text-red-600 transition-colors">
                                    Soy fotógrafo
                                </Link>
                                <Link :href="route('login')" class="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 hover:text-white transition-colors">
                                    Ingresar
                                </Link>
                                <Link :href="route('register')" class="px-6 py-2.5 text-[10px] font-bold uppercase tracking-[0.2em] border border-white text-white hover:bg-white hover:text-black transition-all duration-300 rounded-sm">
                                    Registrarse
                                </Link>
                            </template>
                            ...
                            <template v-else>
                                <div class="flex items-center space-x-4">
                                    <Link v-if="dashboardInfo?.single" :href="dashboardInfo.route" class="text-[10px] font-bold uppercase tracking-[0.2em] text-red-600 hover:text-white transition-colors">
                                        {{ dashboardInfo.text }}
                                    </Link>

                                    <div v-else class="relative">
                                        <button @click="userMenuOpen = !userMenuOpen" class="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 hover:text-white transition-colors">
                                            {{ user.name }}
                                            <ChevronDownIcon :class="['w-3 h-3 transition-transform duration-300', userMenuOpen ? 'rotate-180 text-red-600' : '']" />
                                        </button>
                                        ...
                                        <transition enter-active-class="transition duration-300 ease-out"
                                            ...
                                            <div v-show="userMenuOpen" @click.away="userMenuOpen = false"
                                                ...
                                                <div class="px-5 py-4 text-[10px] text-gray-500 border-b border-white/10 truncate tracking-widest">
                                                    {{ user.email }}
                                                </div>
                                    ...
                                    <Link v-if="dashboardInfo?.single" :href="route('logout')" method="post" as="button" class="text-[10px] font-bold uppercase tracking-[0.2em] text-red-600 hover:text-white transition-colors">
                                        Salir
                                    </Link>
                                </div>
                            </template>
                    ...
                    <button @click="mobileMenuOpen = !mobileMenuOpen" class="md:hidden z-50 focus:outline-none p-2">
                        <div class="space-y-1.5 relative w-6 h-4">
                            <span :class="['absolute w-6 h-[2px] bg-white transition-all duration-300', mobileMenuOpen ? 'rotate-45 top-2' : 'top-0']"></span>
                            <span :class="['absolute w-6 h-[2px] bg-white transition-all duration-300', mobileMenuOpen ? 'opacity-0 top-2' : 'top-2']"></span>
                            <span :class="['absolute w-6 h-[2px] bg-white transition-all duration-300', mobileMenuOpen ? '-rotate-45 top-2' : 'top-4']"></span>
                        </div>
                    </button>
                </div>
            </div>
            ...
            <transition enter-active-class="transition duration-400 ease-out"
                ...
                <div v-show="mobileMenuOpen" class="absolute top-0 left-0 w-full h-screen bg-zinc-950 shadow-2xl pt-28 pb-10 px-8 border-b border-white/10 md:hidden flex flex-col overflow-y-auto">
                    <div class="flex flex-col space-y-8 text-center flex-1">
                        
                        <Link href="/" class="text-sm font-bold uppercase tracking-[0.2em] text-white hover:text-red-600">Inicio</Link>

                        <div class="flex flex-col space-y-4">
                            <span class="text-[10px] font-bold uppercase tracking-[0.3em] text-red-600">Eventos</span>
                            <Link :href="route('events.index')" class="text-sm tracking-widest text-gray-400 hover:text-white">Vigentes</Link>
                            <Link :href="route('future-events.map')" class="text-sm tracking-widest text-gray-400 hover:text-white">Próximos</Link>
                        </div>

                        <Link :href="route('gallery.index')" class="text-sm font-bold uppercase tracking-[0.2em] text-white hover:text-red-600">Galería</Link>
                        <Link :href="route('photographers.index')" class="text-sm font-bold uppercase tracking-[0.2em] text-white hover:text-red-600">Fotógrafos</Link>

                        <Link v-if="user" :href="route('cart.index')" class="text-sm font-bold uppercase tracking-[0.2em] text-red-600 flex items-center justify-center gap-3">
                            <ShoppingCartIcon class="w-5 h-5" />
                            Carrito
                            <span v-if="cartCount > 0" class="bg-red-600 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ cartCount }}</span>
                        </Link>

                        <div class="h-px w-1/3 mx-auto bg-white/20 my-4"></div>

                        <template v-if="!user">
                            <Link :href="route('login')" class="text-xs font-bold tracking-widest uppercase text-gray-400 hover:text-white">Iniciar Sesión</Link>
                            <Link :href="route('register')" class="text-xs font-bold tracking-widest uppercase text-red-600 border border-red-600 py-3 mx-10 rounded-sm hover:bg-red-600 hover:text-white transition-colors">Crear Cuenta</Link>
                        </template>

                        <template v-else>
                            <Link v-if="dashboardInfo?.single" :href="dashboardInfo.route" class="text-xs font-bold tracking-widest uppercase text-red-600">
                                {{ dashboardInfo.text }}
                            </Link>
                            <template v-else>
                                <Link v-for="item in dashboardInfo.items" :key="item.route" :href="item.route" class="text-xs font-bold tracking-widest uppercase text-gray-400 hover:text-white">
                                    {{ item.text }}
                                </Link>
                            </template>
                            <Link :href="route('logout')" method="post" class="text-xs font-bold tracking-widest uppercase text-red-600 mt-4">
                                Cerrar Sesión
                            </Link>
                        </template>
                    </div>
        ...
        <footer class="bg-black text-gray-400 border-t border-white/10">
            <div class="max-w-7xl mx-auto px-8 md:px-16 py-20">
                <div class="flex flex-col md:flex-row justify-between items-center md:items-start space-y-10 md:space-y-0">
                    ...
                    <div class="flex flex-wrap justify-center gap-8 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400">
                ...
                <div class="mt-16 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center text-[11px] tracking-wider text-gray-500">
