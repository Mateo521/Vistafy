...
const allPhotos = ref(props.photos.data); // Array acumulativo
const nextUrl = ref(props.photos.next_page_url); // URL de la siguiente página
const loadingMore = ref(false);

// Si los filtros cambian (props.photos se actualiza), reiniciamos la lista
watch(() => props.photos, (newPhotos) => {
    allPhotos.value = newPhotos.data;
    nextUrl.value = newPhotos.next_page_url;
});
...
const loadMore = () => {
    ...
    router.visit(nextUrl.value, {
        preserveState: true,
        preserveScroll: true,
        only: ['photos'], // Solo pedimos los datos nuevos
        onSuccess: (page) => {
            const newPhotos = page.props.photos.data;
            allPhotos.value = [...allPhotos.value, ...newPhotos]; // Acumulamos
...
const applyFilters = () => {
    // Al filtrar, Inertia recargará la página y el watcher reiniciará allPhotos
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div v-if="allPhotos.length > 0">
                    ...
                    <div v-if="nextUrl" class="flex justify-center pt-8 border-t border-gray-100">
                        ...
                        >
                            <svg v-if="loadingMore" class="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
