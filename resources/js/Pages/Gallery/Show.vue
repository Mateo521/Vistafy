...
const submitReport = () => {
    ...
    reportForm.post(route('gallery.report', props.photo.id), {
        preserveScroll: true,
        onSuccess: () => {
    ...
    <AppLayout>
            ...
            <div class="max-w-[90rem] mx-auto px-4 md:px-8 pt-32 md:pt-40">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
                    ...
                    <div class="lg:col-span-8">
                        <div
                            class="bg-white p-3 md:p-4 rounded shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative group flex items-center justify-center h-full min-h-[50vh]">
                            ...
                            <ProtectedImage :src="photo.watermarked_url || photo.thumbnail_url" :alt="photo.title"
                                ...
                                @click="showFullImage = true" @error="handleImageError" />
                    ...
                    <div class="lg:col-span-4 flex flex-col">
                        ...
                        <div
                            class="bg-white rounded p-8 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] h-full flex flex-col">


                            <div class="mb-6">
                                <span
                                    class="text-[10px] font-bold uppercase text-red-600 bg-red-50 px-4 py-2 rounded-full tracking-widest">
                                    Ref: {{ photo.unique_id }}
                                </span>
                            </div>
                            ...
                            <div class="flex flex-col gap-4 mb-10">
                                <div class="flex justify-between items-center pb-4 border-b border-slate-100">
                                    <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">Resolución
                                        Original</span>
                                    <span class="text-sm font-bold text-slate-800">{{ photo.width }} x {{ photo.height
                                    }} px</span>
                                </div>
                                <div v-if="photo.event"
                                    class="flex justify-between items-center pb-4 border-b border-slate-100">
                                    <span
                                        class="text-xs font-bold text-slate-400 uppercase tracking-wider">Evento</span>
                                    <span class="text-sm font-bold text-red-600 truncate max-w-[150px] text-right">
                                        <Link :href="route('events.show', photo.event.slug || photo.event.id)"
                            ...
                            <div class="mb-8 flex items-end justify-between">
                                <span class="text-xs font-bold uppercase text-slate-400 tracking-wider mb-1">Licencia
                                    Digital</span>
                            ...
                            <div class="mt-6 text-center">
                                <button @click="showReportModal = true"
                                    class="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-slate-700 transition-colors">
                ...
                <div
                    class="mt-8 bg-white rounded p-6 md:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col sm:flex-row items-center justify-between gap-6">
                    <div class="flex items-center gap-6">
                        <div
                            class="w-16 h-16 rounded-full bg-slate-100 flex-shrink-0 flex items-center justify-center overflow-hidden border-2 border-white shadow-md">
                            <ProtectedImage v-if="photo.photographer?.profile_photo_url"
                                :src="photo.photographer.profile_photo_url" class="w-full h-full object-cover" />
                            <div v-else class="text-slate-400 font-flux text-3xl mt-1">
                                {{ photo.photographer?.business_name?.charAt(0) || 'F' }}
                            </div>
                        </div>
                        <div>
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Fotografía de
                            </p>
                            <p class="text-xl font-flux text-black leading-none">{{ photo.photographer?.business_name ||
                                'Fotógrafo F33' }}</p>
                        </div>
                    </div>

                    <Link v-if="photo.photographer?.slug" :href="route('photographers.show', photo.photographer.slug)"
                        ...
                        <Link v-for="related in relatedPhotos" :key="related.id"
                            ...
                            <div
                                ...
                                <div class="w-full flex justify-between items-end">
                                    <span class="text-white font-mono text-[10px] tracking-widest">{{ related.unique_id
                                    }}</span>
        ...
        <Transition enter-active-class="transition-opacity duration-300 ease-out" enter-from-class="opacity-0"
            ...
            <div v-if="showFullImage"
                class="fixed inset-0 z-[100] bg-black/95 backdrop-blur-sm cursor-zoom-out flex flex-col"
        ...
        <div v-if="showReportModal"
            ...
            <div class="bg-white rounded p-6 shadow-2xl w-full max-w-md relative animate-fade-in-up">

                <div class="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
                    <h3 class="text-lg font-bold text-black uppercase tracking-wider">Reportar Fotografía</h3>
                    <button @click="showReportModal = false" class="text-gray-400 hover:text-red-600 transition-colors">
                        <XMarkIcon class="w-6 h-6" />
                    </button>
                </div>
                ...
                <form @submit.prevent="submitReport" class="space-y-5">
                    <div>
                        <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Tu email
                            (Obligatorio)</label>
                        <input v-model="reportForm.email" type="email" required
                            class="w-full border-gray-200 rounded focus:ring-red-600 focus:border-red-600 text-sm shadow-sm">
                        <span v-if="reportForm.errors.email" class="text-xs text-red-600 font-bold mt-1">{{
                            reportForm.errors.email }}</span>
                    </div>

                    <div>
                        <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Motivo
                            del reporte</label>
                        <select v-model="reportForm.reason" required
                            class="w-full border-gray-200 rounded focus:ring-red-600 focus:border-red-600 text-sm shadow-sm">
                            <option value="" disabled>Seleccioná una opción...</option>
                            <option value="copyright">Me pertenece / Infracción de copyright</option>
                            <option value="privacy">Aparezco en la foto sin autorización</option>
                            <option value="inappropriate">Contenido explícito o inapropiado</option>
                            <option value="other">Otro motivo</option>
                        </select>
                        <span v-if="reportForm.errors.reason" class="text-xs text-red-600 font-bold mt-1">{{
                            reportForm.errors.reason }}</span>
                    </div>

                    <div>
                        <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Detalles
                            (opcional)</label>
                    ...
                    <div class="mt-8 flex justify-end gap-4">
                        <button type="button" @click="showReportModal = false"
                            class="px-4 py-2 text-[10px] font-bold text-gray-400 hover:text-black uppercase tracking-widest transition-colors">
                            Cancelar
                        </button>
                        <button type="submit" :disabled="reportForm.processing"
                            class="px-6 py-2 bg-black text-white hover:bg-red-600 text-[10px] font-bold rounded shadow hover:shadow-lg uppercase tracking-widest transition-all disabled:opacity-50 flex items-center gap-2">
                            <span v-if="reportForm.processing">Enviando...</span>
                            <span v-else>Enviar denuncia</span>
                        </button>
                    </div>
                </form>
            </div>
