    ...
    <AppLayout>
        <div
            ...
            <div class="max-w-[1400px] mx-auto px-4 md:px-8">
                ...
                <div
                    class="flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-400 mb-8">
                    ...
                    <Link :href="route('events.show', event.slug)"
                        class="hover:text-red-600 transition-colors truncate max-w-[150px] sm:max-w-[300px]">{{
                        event.name }}</Link>
                    <span class="text-slate-300">/</span>
                ...
                <div
                    class="bg-white rounded p-6 md:p-8 shadow-sm border border-slate-100 mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div class="flex items-center gap-6">
                        <div
                            class="w-20 h-20 rounded-full bg-slate-200 border-4 border-white shadow-md overflow-hidden flex-shrink-0 flex items-center justify-center">
                            <img v-if="photographer.profile_photo_url" :src="photographer.profile_photo_url"
                                class="w-full h-full object-cover">
                            <span v-else class="font-flux text-3xl text-slate-400 mt-1">{{
                                photographer.business_name.charAt(0) }}</span>
                        </div>
                        <div>
                            <span
                                class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Galería
                                del fotógrafo</span>
                            <h1 class="font-flux text-4xl md:text-5xl text-black uppercase leading-none mb-2">
                                {{ photographer.business_name }}
                            </h1>
                            <div v-if="photographer.roles && photographer.roles.length > 0"
                                class="flex flex-wrap gap-2">
                                <span v-for="role in photographer.roles" :key="role"
                                    class="text-[10px] font-bold text-red-600 bg-red-50 px-2 py-1 rounded uppercase tracking-widest border border-red-100">
                                    {{ role }}
                                </span>
                            </div>
                        </div>
                ...
                <div v-if="!photos.data || photos.data.length === 0"
                    ...
                    <p class="font-lato text-slate-500">Este profesional no tiene fotos públicas activas para este
                        evento.</p>
                </div>
                ...
                <div v-else>
                    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-16">
                        <Link v-for="photo in photos.data" :key="photo.id"
                            ...
                            <div
                                class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-3 md:p-4">

                                <div class="flex justify-between items-start w-full">
                                    <button @click.prevent="addToCart(photo)"
                                        class="bg-white/90 backdrop-blur-sm text-slate-800 hover:bg-[#E30613] hover:text-white transition-colors p-2 rounded-full shadow-sm flex items-center justify-center"
                                        title="Agregar al carrito">
                                        <svg v-if="addingToCartIds.includes(photo.id)" class="animate-spin w-4 h-4"
                                        ...
                                        <ShoppingCartIcon v-else class="w-4 h-4" stroke-width="2.5" />
                                    </button>
