    ...
    <AppLayout>
        <div class="relative h-[60vh] min-h-[400px] bg-slate-900 overflow-hidden">
            ...
            <div class="absolute top-24 left-0 w-full px-4 sm:px-6 lg:px-8 z-10">
                <Link :href="route('events.index')" class="inline-flex items-center text-white/70 hover:text-white text-xs font-bold uppercase tracking-widest transition-colors">
        ...
        <div class="min-h-screen bg-white border-t border-gray-100">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                        ...
                        <div class="bg-gray-50 p-6 rounded-sm border border-gray-200">
                            ...
                            <div class="space-y-3">
                                <label class="flex items-center cursor-pointer group">
                                    ...
                                    <div class="w-4 h-4 rounded-full border border-slate-300 mr-3 peer-checked:bg-slate-900 peer-checked:border-slate-900 flex items-center justify-center transition-colors">
                                        <div class="w-1.5 h-1.5 bg-white rounded-full opacity-0 peer-checked:opacity-100"></div>
                                    </div>
                                    <span class="text-sm text-slate-600 peer-checked:text-slate-900 peer-checked:font-medium transition-colors">Mostrar Todo</span>
                                </label>
                                ...
                                <label v-for="photographer in photographers" :key="photographer.id" class="flex items-center cursor-pointer group">
                                    ...
                                    <span class="text-sm text-slate-600 peer-checked:text-slate-900 peer-checked:font-medium transition-colors flex items-center gap-2">
                                        {{ photographer.business_name || photographer.user.name }}
                                        <span class="text-[10px] text-slate-400 bg-white border border-gray-200 px-1.5 rounded-sm">{{ photographer.photos_count }}</span>
                                    </span>
                                </label>
                            </div>
                        </div>
                ...
                <div v-if="photos.data && photos.data.length > 0">
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-12">
                        ...
                        >
                            ...
                            <div class="absolute top-2 right-2 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-sm text-[10px] font-bold uppercase tracking-wider text-slate-900 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                ...
                <div v-else class="text-center py-32 border border-dashed border-gray-200 bg-gray-50 rounded-sm">
                    <p class="text-slate-400 font-serif text-lg">No hay fotografías disponibles con los filtros seleccionados.</p>
                    <button 
