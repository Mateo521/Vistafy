                ...
                <div
                    ...
                    <div class="p-8 md:p-12 relative z-10 lg:w-3/4 order-2 lg:order-1 flex flex-col justify-center">
                        ...
                        <div class="flex flex-wrap gap-3 mb-6">
                            ...
                            <span
                                class="bg-gray-100 text-gray-600 border border-gray-200 px-3 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-widest">
                                {{ event.photos_count || 0 }} Fotografías
                            </span>
                        </div>

                        <h1
                            class="font-flux text-4xl md:text-5xl lg:text-6xl text-black leading-none mb-4 tracking-wide">
                            {{ event.name }}
                        </h1>

                        <p v-if="event.description"
                            class="text-gray-500 max-w-xl text-sm md:text-base leading-relaxed mb-8 font-medium">
                            {{ event.description }}
                        </p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-12">
                    ...
                    <Link :href="route('events.face-search', event.slug)"
                        ...
                        <div class="flex items-center gap-5">
                            <div
                                class="w-14 h-14 bg-red-50 rounded-full flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                                <MagnifyingGlassIcon class="w-6 h-6 text-[#E30613]" />
                            </div>
                            <div>
                                <h4
                                    class="font-flux text-2xl text-black group-hover:text-[#E30613] transition-colors leading-none mb-1">
                                    Escáner facial
                                </h4>
                        ...
                        <div
                            class="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center group-hover:bg-[#E30613] transition-colors shrink-0">
                    ...
                    <Link :href="route('events.bib-search', event.slug)"
                        ...
                        <div class="flex items-center gap-5">
                            <div
                                class="w-14 h-14 bg-gray-100 rounded-full flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                        ...
                        <div
                            class="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center group-hover:bg-black transition-colors shrink-0">
                            <ArrowRightIcon class="w-4 h-4 text-gray-400 group-hover:text-white transition-colors" />
                        </div>
                    </Link>
                    ...
                    <div v-for="gallery in galleries" :key="gallery.photographer.id" class="relative">
                        ...
                        <div
                            ...
                            <div class="flex items-center gap-4">
                                <div
                                    class="w-12 h-12 rounded-full bg-white border border-gray-200 shadow-sm overflow-hidden shrink-0 flex items-center justify-center">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4">
                            <div v-for="(photo, index) in gallery.photos" :key="photo.id"
                                ...
                                <ProtectedImage :src="photo.thumbnail_url"
                                    :alt="`Fotografía de ${event.name} capturada por ${gallery.photographer.business_name}`"
                                    class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 pointer-events-none"
                                    @error="handleImageError" />

                                <div
                                    class="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-3 pointer-events-none">
                                    ...
                                    <div class="flex justify-between items-end w-full">
                                        ...
                                        <div>
                                            <span v-if="photo.location_role"
                                                class="block bg-white/90 backdrop-blur-sm text-black font-bold text-[9px] px-2 py-0.5 rounded uppercase tracking-wider mb-1 w-max">
                                                {{ photo.location_role }}
                                            </span>
                                        ...
                                        <div class="flex flex-col items-end gap-2">
                                            <span
                                                class="bg-[#E30613] text-white font-bold text-[10px] px-2 py-1 rounded-md shadow-sm">
                                                ${{ photo.price }}
                                            </span>
                    ...
                    <swiper :modules="swiperModules" :initialSlide="activeIndex" :navigation="true"
                        ...
                        <swiper-slide v-for="photo in lightboxPhotos" :key="'main-' + photo.id"
                            ...
                            <div class="relative h-full max-w-full flex items-center justify-center">
                                ...
                                <div
                                    class="absolute bottom-4 left-4 right-4 p-4 md:p-6 bg-white/90 backdrop-blur-md rounded shadow-lg flex justify-between items-center border border-white">
                                    <div>
