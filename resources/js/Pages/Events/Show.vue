    ...
    <AppLayout>
        ...
        <div class="min-h-screen bg-gray-50">
            ...
            <div class="relative h-96 bg-gray-900 overflow-hidden">
                <img :src="event.cover_image_url" :alt="event.name" class="w-full h-full object-cover opacity-60"
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <div v-if="event.description" class="bg-white rounded-lg shadow-lg p-8 mb-8">
                    ...
                    <p v-if="event.long_description" class="text-gray-700 leading-relaxed mt-4">{{
                        event.long_description }}</p>
                </div>
                ...
                <div v-if="photographers && photographers.length > 1" class="bg-white rounded-lg shadow-lg p-6 mb-8">
                    ...
                    <div class="flex flex-wrap gap-3">
                        <button @click="selectedPhotographer = 'all'; filterByPhotographer()"
                        ...
                        <button v-for="photographer in photographers" :key="photographer.id"
                ...
                <div v-if="event.photographer" class="bg-white rounded-lg shadow-lg p-6 mb-8">
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            ...
                            <p v-if="event.photographer.region" class="text-sm text-gray-600">{{
                                event.photographer.region }}</p>
                            <div class="mt-3 flex flex-wrap gap-4">
                                <a v-if="event.photographer.phone" :href="`tel:${event.photographer.phone}`"
                                ...
                                <a v-if="event.photographer.email" :href="`mailto:${event.photographer.email}`"
                                    class="flex items-center text-sm text-indigo-600 hover:text-indigo-800 transition">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div>
                    ...
                    <div v-if="photos.data && photos.data.length > 0"
                        ...
                        <div class="aspect-square overflow-hidden bg-gray-100 relative">
                            <img :src="photo.thumbnail_url" :alt="photo.title || photo.unique_id"
                                class="w-full h-full object-cover group-hover:scale-110 transition duration-500"
                                @error="(e) => e.target.src = 'https://via.placeholder.com/400?text=Sin+Imagen'" />
                            <!-- Price Badge -->
                            <div
                                class="absolute top-2 right-2 bg-white/95 backdrop-blur px-2 py-1 rounded-full text-xs font-bold text-indigo-600 shadow-lg">
                                ${{ photo.price }}
                            </div>
                            <!-- Photographer Badge (if multiple photographers) -->
                            <div v-if="photographers.length > 1 && photo.photographer"
