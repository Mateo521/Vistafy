    ...
    <AppLayout>
        <div class="min-h-screen bg-[#F8F9FA] text-slate-800 font-sans selection:bg-red-600 selection:text-white pb-20">
            ...
            <div class="pt-32 pb-16 px-4 md:px-8 max-w-7xl mx-auto text-center">
                <span class="font-bold tracking-widest text-red-600 uppercase text-sm mb-4 block">Todos los eventos
                    pasados</span>
                <h1 class="font-flux text-6xl md:text-8xl text-black mb-6 leading-none">
                    Explorá <br class="md:hidden">
                    <span
                        class="bg-gradient-to-r from-red-600 to-black -webkit-background-clip-text text-transparent bg-clip-text">eventos</span>
                </h1>
            ...
            <div class="max-w-[1400px] mx-auto px-4 md:px-8">
                ...
                <section
                    class="bg-white p-6 md:p-8 rounded shadow-[0_8px_30px_rgb(0,0,0,0.04)] mb-12 border border-slate-100">
                    <form @submit.prevent="submitFilters" class="flex flex-col gap-6">
                        ...
                        <div class="relative">
                            ...
                            <button type="submit"
                                class="absolute inset-y-2 right-2 bg-black text-white px-6 rounded font-bold uppercase tracking-wider text-sm hover:bg-red-600 transition-colors">
                                Buscar
                            </button>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div class="relative">
                                <label
                                    class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Fecha
                                    exacta</label>
                            ...
                            <div class="relative">
                                <label
                                    class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Fotógrafo</label>
                                <select v-model="form.photographer_id"
                                    ...
                                    <option v-for="photographer in photographers" :key="photographer.id"
                                        :value="photographer.id">
                                        {{ photographer.business_name || photographer.user?.name }}
                                    </option>
                                </select>
                                <div
                                    class="pointer-events-none absolute inset-y-0 right-0 top-6 flex items-center px-4 text-slate-500">
                            ...
                            <div class="flex items-end">
                                <button type="button" @click="clearFilters"
                                    class="w-full py-3 px-4 bg-white border border-slate-200 text-slate-500 rounded font-bold uppercase tracking-wider text-sm hover:bg-slate-50 hover:text-red-600 transition-colors flex justify-center items-center gap-2">
                ...
                <div class="flex justify-between items-center mb-8 px-2">
                    ...
                    <span v-if="form.search || form.date || form.photographer_id"
                        class="text-red-600 font-bold text-sm bg-red-50 px-3 py-1 rounded-full animate-pulse">
                        Filtros activos
                    </span>
                </div>
                ...
                <div v-if="!events.data || events.data.length === 0"
                    class="text-center py-24 bg-white rounded shadow-sm border border-slate-100">
                    ...
                    <button @click="clearFilters"
                        class="bg-black text-white px-8 py-3 rounded-full font-bold uppercase tracking-wider text-sm hover:bg-red-600 transition-colors">
                ...
                <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-12">
                    <Link v-for="event in events.data" :key="event.id" :href="route('events.show', event.slug)"
                        ...
                        <div class="flex flex-col min-h-[5rem] md:min-h-[6rem] justify-start">
                            <div
                                class="flex items-center gap-2 text-[10px] font-bold text-red-600 uppercase tracking-widest mb-1.5">
                                <CalendarIcon class="w-3.5 h-3.5" />
                                <span>{{ formatDate(event.event_date) }}</span>
                            </div>
                            <h3
                                class="text-3xl md:text-4xl font-black font-flux text-black leading-none group-hover:text-red-600 transition-colors line-clamp-2">
                                {{ event.name }}
                            </h3>
                        </div>
                        ...
                        <div
                            class="relative w-full aspect-[4/3] rounded overflow-hidden shadow-sm group-hover:shadow-2xl transition-all duration-500 bg-slate-100 mt-1">
                            ...
                            <div
                                class="absolute top-4 right-4 bg-white/90 backdrop-blur-sm text-black font-bold px-3 py-1.5 rounded-full text-[9px] uppercase tracking-widest shadow-sm">
                        ...
                        <div class="flex items-center justify-between pt-1">
                            <div v-if="event.location"
                                class="flex items-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                            ...
                            <span
                                class="text-black font-bold uppercase text-[10px] tracking-widest flex items-center gap-1.5 group-hover:text-red-600 transition-colors">
                                Ver galería
                ...
                <div v-if="events.data && events.data.length > 0 && events.last_page > 1"
                    ...
                    <div class="flex flex-wrap gap-2 bg-white p-2 rounded shadow-sm border border-slate-100">
                        <template v-for="(link, index) in events.links" :key="index">
                            ...
                            <span v-else v-html="link.label"
                                class="min-w-[40px] h-10 flex items-center justify-center px-4 text-sm font-bold text-slate-300"></span>
                        </template>
                    </div>
                </div>
