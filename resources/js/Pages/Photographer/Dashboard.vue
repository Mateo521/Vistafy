...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-12 bg-black min-h-screen text-white selection:bg-red-600 selection:text-white">
            ...
            <div
                class="fixed inset-0 pointer-events-none bg-[linear-gradient(rgba(255,255,255,.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.03)_1px,transparent_1px)] bg-[size:32px_32px] z-0">
            </div>

            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 relative z-10">

                <div
                    class="flex flex-col md:flex-row md:items-end justify-between mb-10 border-b border-red-600 pb-6 gap-4">
                    <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                    ...
                    <div
                        class="bg-zinc-950 p-6 border border-white/10 flex flex-col justify-between h-32 group hover:border-red-600 transition-colors relative overflow-hidden">
                        <div
                            class="absolute top-0 right-0 w-8 h-8 border-t-2 border-r-2 border-white/10 group-hover:border-red-600 transition-colors">
                        </div>
                        <div
                            class="absolute bottom-0 left-0 w-8 h-8 border-b-2  border-white/10 group-hover:border-red-600 transition-colors">
                        </div>

                        <div class="flex justify-between items-start relative z-10">
                            <span
                                class="text-[10px] font-mono uppercase tracking-widest text-gray-500 group-hover:text-red-600 transition-colors">Total
                                Archivo</span>
                            <PhotoIcon class="w-4 h-4 text-gray-700 group-hover:text-red-600" />
                        </div>
                        <span class="text-5xl font-sans font-black text-white tracking-tighter relative z-10">{{
                    ...
                    <div
                        ...
                        <div class="flex justify-between items-start relative z-10">
                            <span
                                class="text-[10px] font-mono uppercase tracking-widest text-gray-500 group-hover:text-red-600 transition-colors">Públicas</span>
                    ...
                    <div
                        ...
                        <div class="flex justify-between items-start relative z-10">
                            <span
                                class="text-[10px] font-mono uppercase tracking-widest text-gray-500 group-hover:text-red-600 transition-colors">Descargas</span>
                    ...
                    <div
                        ...
                        <div class="flex justify-between items-start relative z-10">
                            <span
                                class="text-[10px] font-mono uppercase tracking-widest text-gray-500 group-hover:text-red-600 transition-colors">Misiones
                                / Eventos</span>
                ...
                <div v-if="pendingInvitations && pendingInvitations.length > 0" class="mb-8">
                    <div v-for="invitation in pendingInvitations" :key="invitation.id"
                        class="bg-blue-50 border-l-4 border-blue-600 p-4 rounded-r-lg flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">

                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                <span
                                    class="bg-blue-600 text-white text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded">
                                    Nueva Invitación
                                </span>
                                <span class="text-sm font-bold text-blue-900">{{ invitation.name }}</span>
                            </div>
                            <p class="text-sm text-blue-800">
                                <strong>{{ invitation.photographer.business_name }}</strong> te ha invitado a colaborar
                                como fotógrafo oficial en este evento.
                            </p>
                        </div>
                        ...
                        <div class="flex gap-2 shrink-0">
                            <Link :href="route('photographer.opportunities.reject', invitation.id)" method="post"
                                as="button"
                                class="px-4 py-2 bg-white text-gray-600 border border-gray-200 hover:bg-gray-50 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors">
                                Rechazar
                            </Link>
                            <Link :href="route('photographer.opportunities.accept', invitation.id)" method="post"
                                as="button"
                                class="px-6 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors shadow-sm">
                                Aceptar
                            </Link>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    ...
                    <div class="lg:col-span-2">
                        <div class="flex items-center justify-between mb-6 border-b border-white/20 pb-2">
                            <h3 class="text-xl font-sans font-black uppercase tracking-tighter text-white">Eventos
                                Recientes</h3>
                        ...
                        <div v-if="recentEvents && recentEvents.length > 0" class="space-y-6">
                            <Link v-for="(event, index) in recentEvents" :key="event.id"
                                ...
                                <div
                                    class="w-full md:w-48 h-48 md:h-auto relative overflow-hidden bg-black flex-shrink-0">
                                    ...
                                    <div
                                        class="absolute inset-0 bg-black/40 pointer-events-none group-hover:bg-transparent transition-colors">
                                    </div>
                                ...
                                <div
                                    class="p-6 flex flex-col justify-between flex-1 bg-gradient-to-r from-transparent to-black/20">
                                    <div>
                                        <div class="flex justify-between items-start mb-2">
                                            <h4
                                                class="text-2xl font-sans font-black text-white uppercase tracking-tighter group-hover:text-red-600 transition-colors line-clamp-1">
                                                {{ event.name }}
                                            </h4>
                                            <span
                                                class="text-[10px] font-mono text-gray-500 whitespace-nowrap ml-2 border-b border-gray-700">
                                                {{ formatDate(event.event_date) }}
                                            </span>
                                        </div>
                                        <p v-if="event.description"
                                            class="text-xs text-gray-400 font-mono tracking-wide line-clamp-2 mb-4">
                                            {{ event.description }}
                                        </p>
                                    </div>
                                    ...
                                    <div class="flex items-end justify-between pt-4 border-t border-white/10">
                                        <div
                                            class="flex items-center gap-4 text-[10px] font-mono text-gray-500 uppercase tracking-widest">
                                            <span class="flex items-center">
                                                <PhotoIcon class="w-3.5 h-3.5 mr-1 text-red-600" />
                                                {{ event.photos_count || 0 }} DATA
                                            </span>
                                            <span v-if="event.location"
                                                class="flex items-center truncate max-w-[150px]">
                                                <MapPinIcon class="w-3.5 h-3.5 mr-1 text-red-600" />
                                                {{ event.location }}
                                            </span>
                                        </div>
                                        <span
                                            class="text-[10px] font-mono font-bold uppercase tracking-widest text-white group-hover:text-red-600">>
                                            Ingresar</span>
                                    </div>
                                </div>
                            </Link>
                        ...
                        <div v-else class="text-center py-16 border border-white/10 bg-zinc-950">
                            <p class="text-gray-500 font-mono text-sm uppercase tracking-widest mb-4">Sin operaciones
                                activas.</p>
                            <Link :href="route('photographer.events.create')"
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-6 border-b border-white/20 pb-2">
                            <h3 class="text-xl font-sans font-black uppercase tracking-tighter text-white">Últimos Datos
                            </h3>
                        ...
                        <div v-if="recentPhotos && recentPhotos.length > 0" class="grid grid-cols-2 gap-2">
                            <div v-for="photo in recentPhotos" :key="photo.id"
                                ...
                                <div v-else
                                    class="w-full h-full flex items-center justify-center text-gray-800 bg-zinc-950">
                                    <span class="font-brush text-3xl opacity-20">f33</span>
                                </div>

                                <div
                                    class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center gap-2 pointer-events-none">
                                    <span
                                        class="text-red-600 font-mono text-[9px] uppercase tracking-widest border border-red-600 px-2 py-1 bg-black/80">
                        ...
                        <div v-else class="text-center py-16 border border-white/10 bg-zinc-950">
                            <p class="text-gray-500 font-mono text-xs uppercase tracking-widest mb-4">No hay
                                datos<br>transmitidos.</p>
                            <Link :href="route('photographer.photos.create')"
