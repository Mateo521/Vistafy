...
<template>
    <AppLayout title="Sala de Operaciones">
        ...
        <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 flex flex-col h-[calc(100vh-4rem)]">
            ...
            <div class="bg-white border border-gray-200 rounded-t-lg shadow-sm p-3 md:p-4 flex items-center justify-between z-10 shrink-0 relative">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="text-base font-bold text-black leading-tight truncate max-w-[200px] sm:max-w-xs">{{ event.name }}</h2>
                            <div v-if="event.location" class="flex items-center gap-1 mt-0.5">
                                <MapPinIcon class="w-3 h-3 text-[#E30613]" />
                                <p class="text-[10px] text-gray-500 font-bold uppercase tracking-wider truncate">{{ event.location }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    <div class="hidden sm:block text-[10px] text-gray-400 font-bold uppercase tracking-wider text-right">
                        {{ participants.length }} Equipo
                    </div>
                    <div class="flex -space-x-2.5">
                        <img v-for="p in participants" :key="p.id" 
                            :src="p.profile_photo_url || getFallbackAvatar(p.name)" 
                            :title="p.name"
            ...
            <div class="flex-1 bg-[#f0f2f5] border-x border-gray-200 overflow-hidden flex flex-col relative shadow-inner">
                <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-4">
                    ...
                    <div v-for="msg in messages" :key="msg.id" 
                        ...
                        <div class="flex flex-col max-w-[75%] md:max-w-[65%]" :class="msg.photographer_id === currentPhotographerId ? 'items-end' : 'items-start'">
                            ...
                            <div class="px-4 py-2 text-[15px] leading-relaxed shadow-sm relative"
                                :class="msg.photographer_id === currentPhotographerId 
                                    ? 'bg-[#dcf8c6] text-gray-900 rounded rounded-br-sm'  
                                    : 'bg-white border border-gray-100 text-gray-800 rounded rounded-bl-sm'">
                                {{ msg.message }}
                            </div>
                        </div>
                    </div>
