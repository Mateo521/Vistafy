        ...
        <div class="min-h-screen bg-white">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative -mt-16 pb-12 border-b border-gray-100">
                <div class="flex flex-col md:flex-row gap-8 items-start">
                    ...
                    <div
                        class="w-32 h-32 md:w-40 md:h-40 bg-white p-1 shadow-lg border border-gray-100 flex-shrink-0 rounded-sm">
                        <img v-if="photographer.profile_photo_url" :src="photographer.profile_photo_url"
                            :alt="photographer.business_name" class="w-full h-full object-cover rounded-sm" />
                    ...
                    <div class="flex-1 pt-4 md:pt-16">
                        <div class="flex flex-col lg:flex-row justify-between items-start gap-8">
                            ...
                            <div class="flex-1">
                                ...
                                <div
                                    class="flex flex-wrap items-center gap-4 text-xs font-bold uppercase tracking-widest text-slate-500 mb-6">
                                    <span v-if="photographer.region" class="flex items-center">
                                        <MapPinIcon class="w-4 h-4 mr-1" />
                                        {{ photographer.region }}
                                    </span>
                                    <span
                                        class="flex items-center text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-sm border border-emerald-100">
                                        Verificado
                                    </span>
                                </div>

                                <p v-if="photographer.bio"
                                    class="text-slate-600 font-light max-w-2xl text-sm leading-relaxed mb-8">
                                    {{ photographer.bio }}
                                </p>
                                ...
                                <div class="flex flex-wrap gap-3 border-t border-gray-100 pt-6">
                                    ...
                                    <div class="flex gap-2 ml-2 pl-2 border-l border-gray-200">
                                        ...
                                        <a v-if="photographer.facebook"
                                            :href="`https://facebook.com/${photographer.facebook}`" target="_blank"
                            ...
                            <div
                                class="flex lg:flex-col gap-8 lg:border-l lg:border-gray-200 lg:pl-12 lg:min-w-[150px]">
                                <div>
                                    <span class="block text-3xl font-sans font-bold text-slate-900">{{
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                    ...
                    <div v-if="events.data.length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <Link v-for="event in events.data" :key="event.id" :href="route('events.show', event.slug)"
                            ...
                            <div class="w-2/3 p-6 flex flex-col justify-between">
                                <div>
                                    <span
                                        class="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1 block">
                                        {{ formatDate(event.event_date) }}
                                    </span>
                                    <h3
                                        class="text-xl font-sans font-bold text-slate-900 group-hover:text-slate-600 transition mb-2">
                                        {{ event.name }}
                                    </h3>
                                    <p v-if="event.location"
                                        class="text-xs text-slate-500 flex items-center gap-1 mb-3">
                                        <MapPinIcon class="w-3 h-3" /> {{ event.location }}
                                    </p>
                                </div>
                                <div
                                    class="flex items-center text-xs font-bold uppercase tracking-wider text-slate-900 group-hover:translate-x-2 transition-transform duration-300">
                                    Ver Galería
                ...
                <div v-show="currentTab === 'photos'">
                    ...
                    <div v-if="selectedEventId"
                        class="flex justify-between items-center mb-8 p-4 bg-slate-50 border border-gray-200 rounded-sm">
                        ...
                        <button @click="clearPhotoFilters"
                            class="text-xs font-bold text-slate-900 hover:text-red-600 flex items-center gap-1 transition">
                    ...
                    <div v-if="photos.data.length > 0">
                        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                            ...
                            <div v-for="photo in photos.data" :key="photo.id"
                                ...
                                class="group aspect-square bg-gray-100 overflow-hidden relative border border-gray-100 rounded-sm cursor-pointer">

                                <img :src="photo.watermarked_url" :alt="photo.unique_id" draggable="false"
                                    @contextmenu.prevent
                                    class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 filter grayscale-[0.2] group-hover:grayscale-0 select-none pointer-events-none" />

                                <div
                                    class="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/20 transition-colors duration-300 pointer-events-none">
                                </div>
                            </div>
